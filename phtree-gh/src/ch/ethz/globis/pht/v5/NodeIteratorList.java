...
public class NodeIteratorList<T, R> {
	...
	private final class NodeIterator {
		 ...
		 * @param maxValue
		 */
		private NodeIterator(Node<T> node, long lower, long upper, boolean isDepth0) {
			this.node = node;
	...
	private NodeIteratorList(int DIM, long[] valTemplate, long[] rangeMin,
	...
	static <T, R> ArrayList<R> query(Node<T> node, long[] valTemplate, 
		...
		NodeIteratorList<T, R> i = new NodeIteratorList<>(DIM, valTemplate, rangeMin, rangeMax, 
	...
	private void run(Node<T> node, final boolean isDepth0) {
		...
		new NodeIterator(node, lowerLimit, upperLimit, isDepth0);
	}
