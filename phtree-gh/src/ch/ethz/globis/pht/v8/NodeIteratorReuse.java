...
public class NodeIteratorReuse<T> {
	 ...
	 * @param maxValue
	 */
	public NodeIteratorReuse(int DIM, long[] valTemplate) {
		this.DIM = DIM;
		this.valTemplate = valTemplate;
	}
	
	void reinit(Node<T> node, long[] rangeMin, long[] rangeMax, long lower, long upper) {
	...
	static <T> void init(NodeIteratorReuse<T> ni, long[] rangeMin, long[] rangeMax, 
			long[] valTemplate, Node<T> node) {
		...
		ni.reinit(node, rangeMin, rangeMax, lowerLimit, upperLimit);
	}
