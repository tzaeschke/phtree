...
/**
 * This NodeIterator resuses existing instances, which may be easier on the Java GC.
 * 
 * 
 * @author ztilmann
 *
 * @param <T>
 */
public class NodeIteratorReuse<T> {
	 ...
	 * @param maxValue
	 */
	public NodeIteratorReuse(int DIM, long[] valTemplate, long[] rangeMin, long[] rangeMax) {
		this.DIM = DIM;
	...
	void reinit(Node<T> node, long lower, long upper, boolean isDepth0) {
	...
	static <T> void init(NodeIteratorReuse<T> nIt, long[] rangeMin, long[] rangeMax, 
			long[] valTemplate, Node<T> node, boolean isDepth0) {
		...
		long lowerLimit = 0;
		long upperLimit = 0;
		//TODO combine this with range check incheckAndApplyRange()
		...
		nIt.reinit(node, lowerLimit, upperLimit, isDepth0);
	}
