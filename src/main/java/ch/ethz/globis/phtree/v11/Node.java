...
public class Node {
	...
	/** Bias towards using AHC. AHC is used if: sizeLHC*AHC_LHC_BIAS >= sizeAHC  */
...
//		int offs = pinToOffsBitsData(pin, hcPos, keyToMatch.length);
	...
	private Object insertSplit(long[] newKey, Object newValue, Object currentValue,
			int pin, long hcPos, PhTree11<?> tree, int offs, long mask) {
        //do the splitting
	...
	private boolean useAHC(int entryCount, int dims) {
		...
		return PhTree11.AHC_ENABLED && (dims<=31) && (sizeLHC*AHC_LHC_BIAS >= sizeAHC);
	}
