...
public class NodeIteratorNoGC<T> {
	...
	private void initHCI() {
		...
		if (isNI) {
			...
			if (useHcIncrementer && PhTreeLhc11.HCI_ENABLED && dims < 50) {
				useNiHcIncrementer = true;
			} else {
				useNiHcIncrementer = false;
			}
		} else if (PhTreeLhc11.HCI_ENABLED){
			if (isHC) {
				//nPossibleMatch < 2^k?
				//PAPER
	...
	private boolean readValue(int pin, long pos, PhEntry<T> result) {
		...
		if (o instanceof Node) {
			...
			if (checker != null && sub.getPostLen() < (PhTreeLhc11.DEPTH_64-1) &&
	...
	private boolean getNextHCI(PhEntry<T> result) {
		...
		do {
			if (currentPos == START) {
				...
				currentPos = PhTreeLhc11.inc(currentPos, maskLower, maskUpper);
	...
	boolean adjustMinMax(long[] rangeMin, long[] rangeMax) {
		...
		if (next < this.maskLower) {
			if (isHC) {
				...
				//LHC
				if (this.next + PhTreeLhc11.LHC_BINARY_SEARCH_THRESHOLD < this.maskLower) {
