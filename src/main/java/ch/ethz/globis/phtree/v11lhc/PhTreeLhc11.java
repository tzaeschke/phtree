 ...
 *
 */
public class PhTreeLhc11<T> implements PhTree<T> {
	...
	public PhTreeLhc11(int dim) {
		dims = dim;
		debugCheck();
	}

	public PhTreeLhc11(PhTreeConfig cnf) {
	...
	private PhTreeStats getStats(int currentDepth, Node node, PhTreeStats stats) {
		...
		if (node.values() != null) {
			...
			NodeTreeV11.getStats(node.ind(), stats, dims, entries);
