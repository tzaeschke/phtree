...
//		}
	 ...
	 * @param parentPosStack A stack of positions in parent. [0] contains the highest valid 'pos',
	 * @return the page
	 */
	public BSTreePage findOrCreateSubPage(long key, int[] parentPosStack) {
		//The stored value[i] is the min-values of the according page[i+1} 
        int pos = binarySearch(0, nEntries, key);
        if (pos >= 0) {
            //pos of matching key
        ...
        //read page before that value
        parentPosStack[++parentPosStack[0]] = pos;
        BSTreePage page = readPage(pos);
        if (page != null) {
        	return page;
        }
        return createLeafPage(pos);
	}
     ...
     * @param value
     */
	public final void put(long key, long value, int[] posInParent) {
		if (!isLeaf) {
        ...
        int nEntriesToKeep = (nEntries + newP.nEntries) >> 1;
       	...
       	if (isNew || !isPrev) {
       		if (newP.keys[0] > key) {
       			//posInParent=null, because we won't need it there
       			put(key, value, null);
       		} else {
       			newP.put(key, value, null);
       		}
       	} else {
       		if (keys[0] > key) {
       			newP.put(key, value, null);
