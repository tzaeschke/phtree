 ...
 * B+Tree.
 * 
 * What's wrong with the old NT (nested tree)?
 * - Nested tree is not well suited for high-dim, it will get very deep
 * - Has no infix (fixable), this is bad for deep trees
 * - Minimum of only two (one!) subs per node, this may explain very bad behavior for CU data
 * 
 * 
 * 
 * 
...
public class BPTree<T> {
	...
	public int insert(long hcPos, T data) {
		...
		insert((Chunk)ht, null, hcPos, data);
		return nEntries++;
	}
	...
	@SuppressWarnings("unchecked")
	private T insert(Chunk node, int[] slotId, long hcPos, T data) {
		int pos = Arrays.binarySearch(node.keys, hcPos);
		if (node.level == 0) {
			//leaf
		}
...
//		return ret;
	...
	private static class Chunk {
		...
		Chunk(int level) {
			this.level = level;
			keys = new long[level == 0 ? MAX_N_INNER : (MAX_N_INNER-1)];
		}
	}
