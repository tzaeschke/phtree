 ...
 * @param <R> result type
 */
public class NodeIteratorListReuse3<T, R> {
	
	private class PhIteratorStack {
		@SuppressWarnings("unchecked")
		private final NodeIterator[] stack = new NodeIteratorListReuse3.NodeIterator[64];
		private int size = 0;
	...
	private final PhQueryKnnMbbPPList3<T>.KnnResultList3 results;
	...
	private final class NodeIterator {
		...
		private void niDepthFirstNeighborsSecond() {
			...
			while (niIterator.hasNextULL()) {
				...
				//TODO min/max-check
				if (Long.bitCount(le.getKey() ^ divePos) > 0) {
					LLEntry buf = buffer[bufferSize]; 
					if (buf == null) {
						//TODO use pool
						buf = new LLEntry(0, null);
						buffer[bufferSize] = buf; 
					}
					bufferSize++;
					buf.set(le.getKey(), le.getValue());
				}
			}
	...
	NodeIteratorListReuse3(int dims, PhQueryKnnMbbPPList3<T>.KnnResultList3 results, PhFilterDistance checker) {
		this.dims = dims;
