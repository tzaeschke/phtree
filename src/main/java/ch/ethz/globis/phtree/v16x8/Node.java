...
import ch.ethz.globis.phtree.v16x8.PhTree16x8.UpdateInfo;
import ch.ethz.globis.phtree.v16x8.bst.BSTIteratorAll;
import ch.ethz.globis.phtree.v16x8.bst.BSTPool;
import ch.ethz.globis.phtree.v16x8.bst.BSTreePage;
...
public class Node {
	 ...
	 */
	Object doInsertIfMatching(long[] keyToMatch, Object newValueToInsert, PhTree16x8<?> tree) {
		byte hcPos = toByte(posInArray(keyToMatch, getPostLen()));
	 ...
	 * @return The sub node or null.
	 */
	Object doIfMatching(long[] keyToMatch, boolean getOnly, Node parent, UpdateInfo insertRequired, PhTree16x8<?> tree) {
    ...
    public Node createNode(long[] key1, Object val1, long[] key2, Object val2, int mcb) {
        ...
        byte posSub1 = toByte(posInArray(key1, newPostLen));
        byte posSub2 = toByte(posInArray(key2, newPostLen));
        if (posSub1 < posSub2) {
	...
	private void mergeIntoParentNt(long[] key, Node parent) {
		...
		byte posInParent = toByte(PhTreeHelper.posInArray(key, parent.getPostLen()));
	 ...
	 */
	private void writeEntry(int pin, byte hcPos, long[] newKey, Object value) {
	...
	public final BSTEntry bstGetOrCreate(byte key) {
	...
	public BSTEntry bstRemove(byte key, long[] kdKey, PhTree16x8.UpdateInfo ui) {
	...
	public BSTEntry bstGet(byte key) {
		BSTreePage page = getRoot();
	 ...
	 * @return
	 */
	Object addEntry(byte hcPos, long[] kdKey, Object value) {
		//Uses bstGetOrCreate() -> 
		//- get or create entry
	...
	private Object replaceEntry(byte hcPos, long[] kdKey, Object value) {
	 ...
	 * @return See contract.
	 */
	private Object removeEntry(byte hcPos, long[] key, UpdateInfo ui) {
	...
	BSTEntry getEntry(byte hcPos, long[] keyToMatch) {
		BSTEntry be = bstGet(hcPos);
		if (be == null) {
			return null;
		}
	...
	public static class BSTEntry {
		...
		public BSTEntry(byte key, long[] k, Object v) {
			this.key = key;
			kdKey = k;
			value = v;
		}
		...
		public void set(byte key, long[] kdKey, Object value) {
			this.key = key;
			this.kdKey = kdKey;
			this.value = value;
		}
