...
import ch.ethz.globis.phtree.v16x8.Node.BSTStats;
import ch.ethz.globis.phtree.v16x8.Node.REMOVE_OP;
import ch.ethz.globis.phtree.v16x8.PhTree16x8;
...
public class BSTreePage {
	...
	public BSTreePage findSubPage(byte key) {
	...
	public BSTEntry findAndRemove(byte key, long[] kdKey, Node node, PhTree16x8.UpdateInfo ui) {
		//The stored value[i] is the min-values of the according page[i+1} 
        int pos = binarySearch(key);
        if (pos >= 0) {
            //pos of matching key
	...
	public Object getOrCreate(byte key, Node ind) {
		//The stored value[i] is the min-values of the according page[i+1} 
        int pos = binarySearch(key);
        if (pos >= 0) {
            //pos of matching key
	...
	public BSTEntry getValueFromLeaf(byte key) {
		int pos = binarySearch(key);
		if (pos >= 0) {
            return values[pos];
		}
	...
	int binarySearch(byte key) {
		if (true) return Arrays.binarySearch(keys, 0, nEntries, key);
//		if (nEntries <=6) {
...
//		}
	...
	private int linearSearch(byte key) {
		byte[] keys = this.keys;
	...
	private final void putUnchecked(int pos, byte key, BSTEntry value, Node ind) {
	...
	public final BSTEntry getOrCreate(byte key, BSTreePage parent, int posPageInParent, Node ind) {
		if (!isLeaf) {
	...
	private void updateKey(byte key, int keyPos) {
	...
	private void addSubPage(BSTreePage newP, byte minKey, int keyPos, Node ind) {
		if (isLeaf) {
			throw new IllegalStateException("Tree inconsistency");
		}
		...
		if (nEntries < maxInnerN()) {
			...
			if (keyPos > 0) {
				...
				if (nEntries < 0) {
					...
					byte oldKey = subPages[0].getMinKey();
					if ( minKey > oldKey ) {
						ii = 1;
						keys[0] = minKey;
					} else {
...
//					Arrays.toString(leaves));
	...
	public BSTEntry remove(byte key, long[] kdKey, Node node, PhTree16x8.UpdateInfo ui) {
